...
import { generateAnthropicResponse, generateOpenAIResponse, listAnthropicModels, listOpenAIModels, type AIProvider } from './aiProviders.js';

function providerKey(req: Request, provider: AIProvider) {
  if (provider === 'openai') return ((req.headers['x-openai-api-key'] as string | undefined)?.trim() || process.env.OPENAI_API_KEY || '').trim();
...
export function registerProviderAIRoutes(app: Express) {
  ...
  app.post('/api/ai/chat', async (req: Request, res: Response) => {
    ...
    const primaryModel = typeof req.body?.model === 'string' && req.body.model.trim() ? req.body.model.trim() : (provider === 'openai' ? 'gpt-5.2' : 'claude-sonnet-4-6');
