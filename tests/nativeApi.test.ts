...
test('compiled serverless API starts in native Node ESM without the tsx resolver', async () => {
  ...
  try {
    await build({ entryPoints: ['api/index.ts', 'lib/auth.ts', 'lib/geminiErrors.ts', 'lib/geminiChat.ts', 'lib/geminiModels.ts', 'lib/aiProviders.ts', 'lib/aiRoutes.ts'], outbase: '.', outdir: output, platform: 'node', format: 'esm', bundle: false });
